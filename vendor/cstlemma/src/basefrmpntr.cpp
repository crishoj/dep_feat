...
#if FREQ24
baseformpointer::baseformpointer(const char * s,const char * t,int len,/*int cnt,*/unsigned int frequency):owning(true),next(NULL),hidden(false)
    {
...
#else
#if PFRQ
baseformpointer::baseformpointer(const char * s,const char * t,int len,/*int cnt,*/unsigned int frequency)
...
#else
baseformpointer::baseformpointer(const char * s,const char * t,int len):owning(true),next(NULL),hidden(false)
    {
    bf = new basefrm(s,t,*this,len);
#ifdef COUNTOBJECTS
    ++COUNT;
...
#if PFRQ || FREQ24
int baseformpointer::addBaseForm(const char * s,const char * t,int len,/*int cnt,*/unsigned int frequency)
    {
    if(!bf->equal(s,t))
        {
        if(next)
...
#else
int baseformpointer::addBaseForm(const char * s,const char * t,int len)
    {
    if(!bf->equal(s,t))
        {
        if(next)
